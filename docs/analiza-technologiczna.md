...
Zwolnienie portu 8000:
```bash
lsof -nP -iTCP:8000 -sTCP:LISTEN 2>/dev/null | awk 'NR>1 {print $2}' | xargs -r kill -9
```
...
## 9. Graf dzialania aplikacji
Ponizszy graf pokazuje przeplyw danych od nagrywania audio do prezentacji wyniku w GUI.

```mermaid
flowchart LR
	U[Uzytkownik] -->|Start Recording| GUI[Aplikacja desktopowa GUI]
	GUI --> MIC[Mikrofon]
	MIC --> CAP[Przechwytywanie probek audio]
	CAP --> BUF[Buforowanie sygnalu]
	BUF --> WIN[Podzial na okna 10 s]
	WIN --> WAV[Zapis tymczasowego pliku WAV]
	WAV --> HTTP[POST /predict-window]

	HTTP --> API[Backend FastAPI]
	API --> PRE[Preprocessing sygnalu]
	PRE --> FEAT[Ekstrakcja cech audio]
	FEAT --> SCALE[Normalizacja cech scalerem]
	SCALE --> MODEL[Inferencja modelu Keras]
	MODEL --> RESP[JSON: label, confidence, apnea_probability]

	RESP --> GUI
	GUI --> HIST[Aktualizacja historii predykcji]
	GUI --> CHART[Aktualizacja wykresu trendu]
	GUI --> STATUS[Status nagrywania i predykcji]

	U -->|Stop Recording| GUI
```
